...
#			my_sed( "VNET_BRIDGE=\"xenbr0\"", "VNET_BRIDGE=\"$ENV{'PUB_INTERFACE'}\"", $euca_conf );
        ...
        if( does_It_Have($ROLL, "CLC") && !does_It_Have($ROLL, "SC") ){
                my_sed( 'CLOUD_OPTS="', 'CLOUD_OPTS="--remote-storage ', $euca_conf );
        }
...
#		};
	...
	if(  is_add_disable_iscsi_from_memo() ){
		system("echo \"DISABLE_ISCSI=\\\"N\\\"\" >> $euca_conf ");
	};
...
sub is_add_disable_iscsi_from_memo{
	if( $ENV{'QA_MEMO'} =~ /ADD_DISABLE_ISCSI_N=YES/ ){
		my $extra = $1;
		$extra =~ s/\r//g;
		print "FOUND in MEMO\n";
		print "ADD_DISABLE_ISCSI=YES\n";
		$ENV{'QA_MEMO_ADD_DISABLE_ISCSI_N'} = "YES";
		return 1;
	};
	return 0;
};
